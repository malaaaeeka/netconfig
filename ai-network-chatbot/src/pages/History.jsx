...
function CollapsedSidebar({ user, onNewChat, onToggle }) {
  ...
  const IconBtn = ({ title, onClick, children }) => (
    <div style={{ position: 'relative' }} onMouseEnter={() => setTooltip(title)} onMouseLeave={() => setTooltip(null)}>
      <button
        onClick={onClick}
        style={{ width: '36px', height: '36px', background: 'none', border: 'none', borderRadius: '8px', cursor: 'pointer', color: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s, color 0.15s' }}
        onMouseEnter={e => { e.currentTarget.style.background = '#141414'; e.currentTarget.style.color = '#d1d5db' }}
        onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = '#6b7280' }}
      >{children}</button>
      {tooltip === title && (
        <div style={{ position: 'absolute', left: '46px', top: '50%', transform: 'translateY(-50%)', background: '#141414', border: '1px solid #262626', borderRadius: '6px', padding: '4px 10px', fontSize: '12px', color: '#d1d5db', whiteSpace: 'nowrap', zIndex: 999, pointerEvents: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.4)' }}>{title}</div>
      )}
    </div>
  )
  return (
    <div style={{ width: '52px', background: '#0a0a0a', borderRight: '1px solid #1a1a1a', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0 12px', gap: '2px', height: '100vh', flexShrink: 0 }}>
      <IconBtn title="Open sidebar" onClick={onToggle}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" stroke="currentColor" strokeWidth="1.5"/><line x1="9" y1="3" x2="9" y2="21" stroke="currentColor" strokeWidth="1.5"/></svg>
      </IconBtn>
      <IconBtn title="New chat" onClick={onNewChat}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
      </IconBtn>
      <IconBtn title="Search" onClick={() => navigate('/history')}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" strokeWidth="1.8"/><path d="M16.5 16.5L21 21" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>
      </IconBtn>
      <IconBtn title="Chats" onClick={() => navigate('/chat')}>
      ...
      <div style={{ position: 'relative', cursor: 'pointer' }} onMouseEnter={() => setTooltip('user')} onMouseLeave={() => setTooltip(null)}>
        {user?.photoURL
          ? <img src={user.photoURL} alt="avatar" style={{ width: '32px', height: '32px', borderRadius: '50%', display: 'block' }} />
          : <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '13px', fontWeight: '500' }}>{user?.displayName?.[0] || 'U'}</div>
        }
        {tooltip === 'user' && (
          <div style={{ position: 'absolute', left: '42px', bottom: '0', background: '#141414', border: '1px solid #262626', borderRadius: '6px', padding: '4px 10px', fontSize: '12px', color: '#d1d5db', whiteSpace: 'nowrap', zIndex: 999, pointerEvents: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.4)' }}>{user?.displayName || user?.email || 'User'}</div>
        )}
      </div>
    </div>
  )
...
export default function History() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#0a0a0a', color: '#e5e7eb', fontFamily: 'ui-sans-serif, system-ui, sans-serif' }}>
      {sidebarOpen
        ? <ProtocolSidebar
            selected={protocol}
        ...
        <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
            ...
            <button
              ...
              style={{ background: '#fff', border: 'none', borderRadius: '8px', color: '#000', fontSize: '13px', padding: '8px 16px', cursor: 'pointer', fontWeight: '500', transition: 'opacity 0.15s' }}
          ...
          <div style={{ position: 'relative', marginBottom: '20px' }}>
            <svg style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: '#6b7280' }} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <input
              ...
              style={{ width: '100%', background: '#141414', border: '1px solid #262626', borderRadius: '10px', padding: '12px 14px 12px 40px', color: '#d1d5db', fontSize: '14px', outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.15s' }}
          ...
          {selectMode && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 4px', marginBottom: '4px' }}>
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', padding: '4px', fontSize: '16px' }}
              >✕</button>
            </div>
          )}
          ...
          {loading && (
            <div style={{ display: 'flex', justifyContent: 'center', padding: '60px 0' }}>
              <div style={{ width: '24px', height: '24px', border: '2px solid #262626', borderTop: '2px solid #3b82f6', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          ...
          {!loading && filtered.length > 0 && (
            <div>
              ...
              {filtered.map((chat) => {
                ...
                return (
                  <div key={chat.id} style={{ borderTop: '1px solid #1a1a1a' }}>
                    {renamingId === chat.id ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '14px 4px' }}>
                        <input
                          ...
                          style={{ flex: 1, background: '#141414', border: '1px solid #3b82f6', borderRadius: '6px', padding: '6px 10px', color: '#fff', fontSize: '14px', outline: 'none' }}
                        />
                        <button
                          onClick={() => handleRename(chat.id)}
                          style={{ background: '#3b82f6', border: 'none', borderRadius: '6px', color: '#fff', padding: '6px 12px', cursor: 'pointer', fontSize: '13px' }}
                        >Save</button>
                        <button
                      ...
                      <div
                        ...
                        onMouseEnter={e => { e.currentTarget.style.background = selected.has(chat.id) ? 'rgba(59,130,246,0.12)' : '#141414'; setOpenMenu(chat.id + '_hover') }}
                        ...
                        {!selectMode && (
                          <div style={{ position: 'relative' }} ref={openMenu === chat.id ? menuRef : null}>
                            ...
                            {openMenu === chat.id && (
                              <div
                                ref={menuRef}
                                style={{ position: 'absolute', right: 0, top: 'calc(100% + 4px)', background: '#0a0a0a', border: '1px solid #262626', borderRadius: '10px', zIndex: 100, minWidth: '160px', boxShadow: '0 8px 24px rgba(0,0,0,0.6)', overflow: 'hidden', padding: '4px' }}
                              >
                                {[
